			...
			<div class="col-lg-10 offset-lg-1">
				<div class="shopping_cart">
					...
					<table class="shop_table shop_table_responsive cart" cellspacing="0">
						...
						<tbody>
						<?php $products_total_price = 0; ?>
						@foreach($cart as $cart_data)
						<tr>
							<td class="product-remove">
								<a href="{{route('remove-from-cart', ['product_id' => $cart_data->product_id, 'buyer_id' => session()->get('userId')])}}" class="remove" aria-label="Remove this item" data-product_id="" data-product_sku="">
									×
								</a>
							</td>
							...
							<td class="product-name" data-title="Product">
								<a href="#">{{$cart_data->product->title}}</a>
							</td>
							<td class="product-quantity" data-title="Quantity">
								<div class="quantity buttons_added">
									<input type="button" value="-" class="minus quantity_button" data-id="{{$cart_data->product_id}}" data-buyer="{{session()->get('userId')}}" />
									<input type="text" step="1" min="1" max="" name="quantity" value="{{$cart_data->quantity}}" title="Qty" class="input-text qty text"  size="4" pattern="" inputmode="" />
									<input type="button" value="+" class="plus quantity_button" data-id="{{$cart_data->product_id}}" data-buyer="{{session()->get('userId')}}" />
								</div>
							</td>
							<td class="product-price" data-title="Price">
								<span class="amount">
									<span class="">$</span>{{$cart_data->product->price * $cart_data->quantity}}
								</span>
							</td>
						</tr>
						<?php 
								...
								<tbody>
									...
									<tr class="cart-subtotal">
										...
										<td data-title="Subtotal">
											<span class="amount">
												<span class="">$</span>{{$products_total_price}}
											</span>
										</td>
									</tr>
									<tr class="cart-subtotal">
										...
										<div id="checkout_shipping_details_two">
											...
											<form id="shipping-form">
												...
												<p>
													...
													<input type="email" required class="input-text" name="email" placeholder="e.g. you@email.com" />
												</p>
												...
												<p>
													<label>Name</label>
													<input type="text" required class="input-text" name="name" placeholder="e.g. John Smith" />
												</p>
												<p>
													<label>Phone Number</label>
													<input type="number" required class="input-text" name="phone" placeholder="e.g. 123-456-7890" />
												</p>
												<p>
													<label>Street Address</label>
													<input type="text" required class="input-text" name="street_address_one" placeholder="e.g. 111 Sutter St" />
												</p>
												<p>
													<label>Street Address 2</label>
													<input type="text" required class="input-text" name="street_address_two" placeholder="Apartment, suite, unit, building, floor, etc." />
												</p>
												<p>
													<label>ZIP or Postcode</label>
													<input type="text" required class="input-text" name="post_code" placeholder="e.g. 94104" />
												</p>
												<p>
													<label>City or Suburb</label>
													<input type="text" required class="input-text" name="city" placeholder="e.g. San Francisco" />
												</p>
												<p>
													<label>State, Region or Province</label>
													<input type="text" required class="input-text" name="state" placeholder="e.g. California" />
												</p>
												<p class="continue_btn">
													<a class="continue_button btn submit-shipping-address" href="javascript:void(0)">Continue</a>
												</p>
											</form>
										</div>
									</div>
								...
								<div id="checkout_stap_four" class="checkout_stap">
									...
									<div class="checkout_stap_text">
										...
										<div id="checkout_credit_card_two">
											...
											<form class="credit_card_form require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{ env('STRIPE_KEY') }}" id="payment-form" role="form" action="{{ route('stripe.post') }}" method="post">
												@csrf
												<p>
												...
												<p>
													...
													<input type="text" class="input-text card-number" autocomplete="off" placeholder="1234 1234 1234 1234" />
												</p>
												<p>
													<label>Expiry Month</label>
													<input autocomplete='off' type="text" class="input-text card-expiry-month" size="2" placeholder="MM" />
												</p>
												
												<p>
													<label>Expiration Year</label>
													<input autocomplete='off' type="text" class="input-text card-expiry-year" placeholder='YYYY' size='4' />
												</p>

												<p>
													<label>CVC</label>
													<input autocomplete='off' type="text" class="input-text card-cvc" placeholder="CVC" />
												</p>
												<button type="submit" class="btn">Pay <span>${{$products_total_price}}</span></button>
											</form>
											<ul class="Pay_icon">
...
<script>
	$(document).ready(function() {
		...
		$('.minus').click(function(e) {
			...
			var buyer_id = $(this).data('buyer');

			var data = {product_id, buyer_id, '_token': "{{csrf_token()}}"};
			var route = "{{route('decrement-cart')}}";
			var type = "GET";

			makeAjaxCall(data, route, type);
		});
		...
		$('.plus').click(function(e) {
			...
			var data = {product_id, buyer_id, '_token': "{{csrf_token()}}"};
			var route = "{{route('increment-cart')}}";
			var type = "GET";
		...
		$('.submit-shipping-address').click(function() {
			...
			var route = "{{route('save-shipping-address')}}";
			var type = "POST";

			makeAjaxCall(data, route, type);
		});

		function makeAjaxCall(data, route, type) {
			$.ajax({
			url : route,
			data : data,
			type : type,
